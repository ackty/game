...
enum Move { MOVE_UP = 0, MOVE_DOWN = 1, MOVE_LEFT = 2, MOVE_RIGHT = 3, EXIT = 255};
...
int main(void){
    ...
    while( !Game_isFinished() ){ 
        if( Game_update( Controller_getMove() ) == -2 ) goto game_quit; 
        View_showBoard(); 
    }
